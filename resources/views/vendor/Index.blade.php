...
<link rel="stylesheet" href="{{ asset('css/style_semuaToko.css') }}">
	...
	<div class="container" >
		...
		<div class="row mb-3">
			...
			<!-- REPEAT -->
			@foreach ($toko as $vendor=>$row)
				<div class="col">
					<div class="card mb-5" style="max-width: 100%; border-radius: 20px;">
						<div class="row g-0">
							<div class="col-md-4">
								<img src="{{ URL::asset('image/'.$row->image)}}" class="img-fluid rounded-start" alt="...">
							</div>
							<div class="col-md-8">
								<div class="card-body">
									<h5 class="card-title">{{ $row->store_name }}</h5>
									<p class="card-text">{{ $row->store_name }}</p>
									<p class="card-text"><a href="{{ url('toko/'.$row->id) }}" style="text-decoration: none; font-weight: 450; font-size: 18px;">Kunjungi Toko ></a></p>
								</div>
							</div>
						</div>
					</div>
